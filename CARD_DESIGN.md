...
- 魔法/陷阱/装备的"攻/血"为 **P1 临时数值**（P1 全部按召唤卡结算），P2 实现技能后不再参与站场
...
- **装备**：附着己方召唤物，**每召唤物 1 件，新装备替换旧装备**（旧装备进弃牌堆）；装备 ATK 提供攻击加成，装备 HP 作为**耐久**先吸收伤害（归零损毁进弃牌堆，MC 护甲/耐久设定）
...
- equip 类：P1 全部=数值+耐久（特殊装备技能 `equip_elytra`/`equip_trident`/`equip_ranged`/`equip_taunt`/`equip_guard` 留 P2）
